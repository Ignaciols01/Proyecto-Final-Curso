  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto transition-colors duration-300">
      ...
      <div className="text-center mb-10">
        <h1 className="text-3xl font-extrabold text-blue-600 dark:text-blue-500 mb-2">Control Horario</h1>
        <p className="text-slate-500 dark:text-gray-400 font-medium">Registra tu jornada laboral con precisión</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-[#1e293b] rounded-3xl p-8 flex flex-col items-center justify-center border border-slate-200 dark:border-slate-700 shadow-xl transition-colors duration-300">
          ...
          {(!puedeFicharEntrada && !puedeFicharSalida) && mensajeBloqueo && (
            <p className="mt-6 text-sm font-bold text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-400/10 px-4 py-2 rounded-lg text-center animate-fade-in">
              {mensajeBloqueo}
            </p>
          )}
        </div>
        ...
        {/* RESUMEN */}
        <div className="bg-white dark:bg-[#1e293b] rounded-3xl p-8 border border-slate-200 dark:border-slate-700 shadow-xl flex flex-col transition-colors duration-300">
          <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-6">Resumen del Turno</h2>
          
          <div className="space-y-4 mb-8">
            <div className="bg-slate-50 dark:bg-[#0f172a]/50 rounded-xl p-4 flex items-center justify-between border border-slate-200 dark:border-emerald-900/30">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-emerald-500"></div>
                <span className="font-bold text-slate-700 dark:text-gray-200">Entrada</span>
              </div>
              <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400 text-lg">{formatTimeSmall(horaEntrada)}</span>
            </div>

            <div className="bg-slate-50 dark:bg-[#0f172a]/50 rounded-xl p-4 flex items-center justify-between border border-slate-200 dark:border-red-900/30">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-red-500"></div>
                <span className="font-bold text-slate-700 dark:text-gray-200">Salida</span>
              </div>
              <span className="font-mono font-bold text-red-600 dark:text-red-400 text-lg">{formatTimeSmall(horaSalida)}</span>
            </div>
          </div>

          <div className="mt-auto space-y-4">
            <div className="bg-blue-50 dark:bg-[#334155]/30 rounded-xl p-5 border border-blue-100 dark:border-slate-700/50">
              <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-1">Horas de este turno</p>
              <p className="text-2xl font-black text-blue-700 dark:text-blue-400/80 font-mono tracking-wide">{horasComputadasTurno}</p>
            </div>

            <div className="bg-purple-50 dark:bg-purple-900/10 rounded-xl p-5 border border-purple-100 dark:border-purple-900/30">
              <p className="text-[10px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-1">Total Acumulado (Histórico)</p>
              <p className="text-2xl font-black text-purple-700 dark:text-purple-400/80 font-mono tracking-wide">{horasTotalesAcumuladas}</p>
            </div>
          </div>
        </div>
      </div>
